...
REM 部署运行时（标准产物名）：
REM - runq_deploy_cpu.exe
REM - runq_deploy_hw.exe
REM - runq_verify.exe
...
REM 资产导出（等价于 make export_deploy_assets）：
REM python tools\export_deploy_headers.py --model-bin artifacts\stories260K\stories260K_q80.bin --tokenizer-bin artifacts\stories260K\tok512.bin
